...
static void weakScaling(benchmark::internal::Benchmark* b){
    const int nnz_per_row = 16;

    for(long rows : {100, 500, 1000, 5000, 10000, 50000, 100000}){
        long cols = rows;
...
static void strongScaling(benchmark::internal::Benchmark* b){
    long rows = 10000, cols = 10000;

    for(long nnz : {1, 2, 4, 8, 16}){
