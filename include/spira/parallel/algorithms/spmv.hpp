...
namespace spira::parallel::algorithms
{
    ...
    //
    // The partition CSR is always built once the matrix is locked.
    // ─────────────────────────────────────────────────────────────────────────────

    template <class L, concepts::Indexable I, concepts::Valueable V, class BT,
              std::size_t BN, config::insert_policy IP,
              std::size_t SN>
        ...
        mat.execute([&x, &y](const auto &p, std::size_t)
        {
            ...
            else // aos_tag: interleaved (col, val) pairs
            {
                ...
                for (std::size_t i = 0; i < p.size(); ++i)
                {
                    V acc = traits::ValueTraits<V>::zero();
                    for (std::size_t k = off[i]; k < off[i + 1]; ++k)
                        acc += x[static_cast<std::size_t>(pairs[k].column)] * pairs[k].value;
                    y[p.row_start + i] = acc;
                }
            }
        });
    ...
    inline void spmv(parallel_matrix<layout::tags::soa_tag, uint32_t, float, BT, BN, IP, SN> &mat,
