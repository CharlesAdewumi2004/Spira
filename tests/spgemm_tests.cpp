...
// Helper: build a sparse matrix from triplets (i,j,val)
...
// Pick a Layout that exists in your library.
// If you have tags like layout::csr or layout::row_major_sparse, swap here.
using LayoutTag = spira::layout::tags::aos_tag;
using I = size_t;

// 1) Dimension mismatch throws
...
{
    using V = int;
    auto A = make_mat<LayoutTag,I,V>(3, 4, {
        {0,1,2}, {2,3,5}
    });
...
// 3) Identity: A * I = A (shape permitting)
TEST(SpGEMM, RightIdentity)
{
    using V = int;
    auto A = make_mat<LayoutTag,I,V>(3, 3, {
        {0,0,1}, {0,2,4},
        {1,1,3},
        {2,0,-2}, {2,2,7}
    });

    auto Iden = make_mat<LayoutTag,I,V>(3, 3, {
        {0,0,1}, {1,1,1}, {2,2,1}
    });
...
TEST(SpGEMM, SmallExampleMatchesReference)
{
    ...
    // [1 0 2]
    // [0 3 0]
    auto A = make_mat<LayoutTag,I,V>(2, 3, {
        {0,0,1}, {0,2,2},
        {1,1,3}
    });
    ...
    // [5 0]
    // [0 6]
    auto B = make_mat<LayoutTag,I,V>(3, 2, {
        {0,1,4},
        {1,0,5},
        {2,1,6}
    });
...
{
    using V = int;
    auto A = make_mat<LayoutTag,I,V>(4, 3, {
        {0,0,1}, {0,2,2},
        {2,1,3},
        {3,0,4}
    });

    auto B = make_mat<LayoutTag,I,V>(3, 5, {
        {0,1,7}, {0,4,1},
        {1,2,2},
        {2,1,3}, {2,3,5}
    });
...
TEST(SpGEMM, NonCommutativeSanity)
{
    ...
    // Just check at least one entry differs (if your get() returns 0 for missing)
    bool any_diff = false;
    for (int i = 0; i < 2; ++i)
        for (int j = 0; j < 2; ++j)
